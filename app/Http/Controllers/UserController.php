...

{
    ...
    public function edit(Request $request, $id_user)
    {
        $usuario = User::with(['votantes.hashVotantes' => function ($query) {
            // Cargar solo el campo `tipo` de hashVotantes
            $query->select('id', 'tipo', 'id_evento', 'id_votante', 'subtipo'); // Ajusta los campos según tu estructura
        }])->findOrFail($id_user);
        ...
        $usuario->subtipo = optional($usuario->votantes) // Aseguramos que votantes exista
            ->hashVotantes->pluck('subtipo') // Extraemos los `subtipo` de cada `hashVotante`
            ->flatMap(function ($subtipo) {
                return explode('|', $subtipo); // Dividir cada `subtipo` en subtipos separados por '|'
            })
            ->unique() // Elimina valores duplicados
            ->implode('|');
        ...
        return Inertia::render('Users/Edit', compact('usuario', 'roles', 'userRole', 'tipos', 'eventos', 'parametros', 'subtipos'));
    }
