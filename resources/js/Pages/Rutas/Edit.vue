  ...
  <AuthenticatedLayout :breadCrumbLinks="breadcrumbLinks">
    ...
    <div class="w-full grid md:grid-cols-3 gap-4">
      <div class="md:col-span-2 p-4 bg-white rounded-md shadow-md h-full">
        <h2 class="text-lg font-bold mb-4">Editar Ruta/path</h2>
      ...
      >
        <div class="p-4 md:p-5 w-full border border-gray rounded-md shadow-sm">
          ...
          <div class="mb-8">
            ...
            <p>
              {{ form.estado === 1 || form.estado === "1" ? "Activo" : "Bloqueado" }}
            </p>
          </div>
        </div>
      </div>
...
const form = useForm({
  path: props.ruta.path,
  fecha_inicio: props.ruta.fecha_inicio,
  fecha_fin: props.ruta.fecha_fin,
  estado: props.ruta.estado,
});

const breadcrumbLinks = [
  { url: "/rutas", text: "Listado de rutas" },
  { url: "", text: "Editar ruta "+props.ruta.path },
];
  ...
  form.patch(route("rutas.update"), {
    onSuccess: () =>
      swal({
        ...
        text: "La ruta se ha actualizado exitosamente",
        icon: "success",
      }).then(() => {
    ...
    onError: () => {
      swal({
        title: "Error",
        text: "Ocurrió un error al actualizar la ruta.",
        icon: "error",
      });
    },
  });
