...
const form = useForm({
  detalle: props.parametro.detalle || "",
  cod: props.parametro.cod || "",
  estado: props.parametro.estado,
  id: props.parametro.id,
});

const breadcrumbLinks = [
  { url: "/parametros", text: "Listado de parámetros" },
  { url: "", text: "Editar parámetro" },
];
...
const validateCode = () => {
  ...
  if (currentCode == props.parametro.cod) {
    isExisting.value = false;
    return;
  }
  const existingCode = props.codigos.find((item) => item.cod === currentCode);

  console.log("props.codigos", props.parametro.cod);

  if (existingCode) {
    console.log("existingCode", existingCode.cod);
...
const submit = () => {
  ...
  form.patch(route("parametros.update", props.parametro.id), {
    onSuccess: () =>
      swal({
