<template>
  ...
  <AuthenticatedLayout :breadCrumbLinks="breadcrumbLinks">
    <template #header> Análisis resultados </template>
    ...
    <div class="">
      <div v-if="eventos.length" class="flex flex-col justify-center">
        ...
        <div
          class="flex inline-flex justify-center gap-4 sm:gap-6 mb-4 mt-6 text-lg sm:mb-auto"
        >
          <button
            v-for="ev in eventos
              .find((item) => item.id == evento_selected)
              .tipos.split('|')"
            :key="ev"
            class="flex cursor mb-2 sm:mb-auto"
            @click="showGraphics(ev)"
        ...
        <div class="sm:grid grid-cols-2 md:gap-6 sm:gap-4 mt-6">
          ...
          <div class="flex h-full m-auto flex-col justify-center mb-4 px-4">
            <h2
              class="text-gray-600 text-lg sm:text-xl font-bold text-center flex justify-center"
            >
                    ...
                    <table class="w-full whitespace-no-wrap">
                      ...
                      <tbody>
                        ...
                        >
                          ...
                          >
                            <Avatar
                              v-if="user.votante.imagen == 'user.png'"
                            ...
                            <Avatar
                              v-else
                              :image="getImageUrl(user.votante.imagen)"
                            ...
                            <p class="text-gray-900 whitespace-no-wrap">
                              {{
                                eventos
                                  ...
                                  .votos.filter(
                                    (item) =>
                                      item.id_candidato == user.id_votante
                                  ).length
                              }}
                            </p>
                          </td>
      ...
      <div v-else class="flex justify-center">
        <Message
          ...
          :life="3000"
          >El análisis de los eventos de votación se mostrará en esta
          sección</Message
        >
      </div>
    </div>
...
//breadcrumb
const breadcrumbLinks = [{ url: "/", text: "Análisis" }];
...
const tipos = ref(
  props.eventos.length
    ? props.eventos
        .find((item) => item.id == evento_selected.value)
        .tipos.split("|")[0]
    : "NA"
);

//candidatos segun tipo
const candidatosXtipo = ref(
  props.candidatos.filter((item) => {
...
const totalPages = computed(() =>
  Math.ceil(candidatosXtipo.value.length / itemsPerPage.value)
);
...
watch(evento_selected, (value) => {
  tipos.value = props.eventos.length
    ? props.eventos.find((item) => item.id == value).tipos.split("|")[0]
    : "NA";
...
const setChartData = () => {
  ...
  //Evento seleccionado
  let eventoSelect = props.eventos.find(
  ...
  candidatosXtipo.forEach((element) => {
    ...
    votosArray.push(
      eventoSelect.votos.filter(
        (item) => item.id_candidato == element.id_votante
      ).length
    );
  });

  return {
    labels: labelsCandidatos
      .join(", ")
      .split(",")
...
const setChartData1 = () => {
  ...
  let votantes = props.votantes.filter(
    (item) => item.tipo == tipos.value && item.id_evento == eventoSelect.id
  ).length;
  let votos = eventoSelect.votos.filter(
    (item) => item.tipo == tipos.value
  ).length;
